...
export async function generatePaymentUrl(product: Product, user: User, metadata: PaymentMetadata): Promise<string> {
  const response = await mercadopagoClient.post(String(process.env.MERCADOPAGO_PREFERENCES_PATH), {
    items: [
      {
        id: product.id,
        title: product.name,
